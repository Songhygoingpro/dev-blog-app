...
document.querySelector(".search-post").addEventListener("input", function () {
  const query = document
    .querySelector(".search-post")
    .value.trim()
    .toLowerCase(); // Get the search query and convert it to lowercase
...
function showPostSuggestions(posts) {
  // Parameter is named `posts` to represent an array of objects
  ...
  posts.forEach((post) => {
    // Loop through `posts` (filteredPosts)
...
document.querySelector(".search-post")
  .addEventListener("click", function (event) {
    ...
    if (
      document.querySelector(".post-suggestion") &&
      document.querySelector(".search-post").value
    ) {
...
//Handle search by text in the field
document.querySelector(".search-post").addEventListener("keydown", function (e) {
    if (e.keystroke === "Enter") {
